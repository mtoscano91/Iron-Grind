...
- [ ] **AC-18** [BLOCKING]: Database initialized with the 34 MVP records — `GetItemsByCategory(ItemCategory.Equipment).Count == 28`; every element has `ItemCategory == Equipment`; no element has `ItemCategory == Consumable`.
- [ ] **AC-24** [BLOCKING]: Every Equipment record has `IsUpgradeable = true`; every Consumable record has `IsUpgradeable = false`; no exceptions.
- [ ] **AC-30** [BLOCKING]: Bronze Sword record: `SellPriceGold == 10` (F-1 minimum at default tuning).
- [ ] **AC-31** [BLOCKING]: DarkSteel Sword record: `SellPriceGold == 270` (F-1 maximum at default tuning).
- [ ] **AC-33** [BLOCKING]: HP Potion Small `SellPriceGold == 2`, HP Potion Medium `SellPriceGold == 6`, HP Potion Large `SellPriceGold == 18`. Same values for MP Potion variants.
- [ ] **AC-34** [BLOCKING]: Exactly 28 records have `ItemCategory == Equipment` and exactly 6 have `ItemCategory == Consumable`. No record holds both categories.
