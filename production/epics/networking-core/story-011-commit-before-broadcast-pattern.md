...
- [ ] **AC-NC-09** [BLOCKING]: Given a simulated database write that takes 200ms to confirm, when a mock irreversible-outcome request is submitted, then the client does not receive the outcome message and does not play any outcome-dependent behavior until at least 200ms after submission, verified by comparing client-side receipt timestamp against server-side write confirmation timestamp.
- [ ] **AC-NC-15** [BLOCKING]: Given test instrumentation (`IServerCrashInjector` delay variant) that delays the persistence write by 500ms, when a mock irreversible-outcome request is submitted, then the client does not receive the outcome until at least 500ms after submission.
