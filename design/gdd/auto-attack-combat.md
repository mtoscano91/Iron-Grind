...
10. When `_actionTimer >= CycleDuration`, the server resolves in this fixed order:
    ...
    - **Step 3 (auto fires):** Call Damage Calculation with `(BaseDamage, AttackerID, TargetID, DamageContext.PhysicalAuto)`. Apply returned `DamageResult.FinalDamage` to target health. Use `DamageResult.IsCrit` for VFX routing and `DamageResult.IsKill` to trigger IDLE transition.
