...
|--------|------|-----------------|-------|
| Auto-Attack Combat | Hard | `DamageCalculation(BaseDamage, AttackerID, TargetID, DamageContext.PhysicalAuto)` → `DamageResult` | On `IsKill = true`: caller must call `GetXPAward(TargetID)`, award XP to all eligible party members (Rule 4a), then call `CharacterStats.ApplyDamage(TargetID, FinalDamage)`. Auto-Attack Combat GDD Step 4 must be updated to include this kill sequence. Rule 18 provisional is closed: no damage reversal supported. |
